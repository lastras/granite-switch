...
Runs _position_zero_nan_tests.py in a subprocess so the parent pytest process
...
_INNER = Path(__file__).parent / "_position_zero_nan_tests.py"
_TIMEOUT = 600
...
class TestExpandControlDimensions:
    def test_suite(self):
        _run_inner_class("TestExpandControlDimensions")


class TestSDPANaN:
    def test_suite(self):
        _run_inner_class("TestSDPANaN")


class TestModelFiniteness:
    def test_suite(self):
        _run_inner_class("TestModelFiniteness")


class TestFixSensitivity:
    def test_suite(self):
        _run_inner_class("TestFixSensitivity")
