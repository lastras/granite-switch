...
"""Upstream equivalence tests (subprocess wrapper).

Runs _upstream_equivalence_tests.py in a subprocess so the parent pytest
...
_INNER = Path(__file__).parent / "_upstream_equivalence_tests.py"
_TIMEOUT = 1500
...
class TestAttentionOnlyNoMoE:
    def test_suite(self):
        _run_inner_class("TestAttentionOnlyNoMoE")
