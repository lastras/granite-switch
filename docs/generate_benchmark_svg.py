...
def make_adapter_entries(slot_indices, row_idx):
    """Generate adapter box fade-in/out entries for given slot indices."""
    entries = []
    for si in slot_indices:
        ...
        entries.append((delay, "opacity:0"))
...
def make_bar_entries(slot_indices, row_idx):
    """Generate bar fade-in/out entries for given slot indices."""
    entries = []
    for si in slot_indices:
        delay = slot_frac(si, BAR_DELAY[row_idx])
        show = slot_frac(si, BAR_START[row_idx])
        hold = slot_frac(si, CONTENT_OUT)
        end = slot_frac(si, SLOT_END)
        entries.append((delay, "opacity:0"))
        entries.append((show, "opacity:1"))
...
def fmt_keyframes(name, entries):
    ...
    Adds visibility:hidden/visible alongside opacity for Safari compatibility.
    """
    seen = {}
    for t, v in entries:
        seen[t] = v
