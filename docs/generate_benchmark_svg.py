...
Uses <animate> for rect width/fill changes and <set> for text display toggling.
No CSS @keyframes or animation properties — pure SMIL.
...
def slot_begin(slot_idx):
    """Return the begin time in seconds for a given slot index."""
    return slot_idx * SLOT_SECONDS


def smil_set(begin_s, dur_s=SLOT_SECONDS):
    """Return a <set> element string for display toggling."""
    return f'<set attributeName="display" to="inline" begin="{begin_s}s" dur="{dur_s}s" repeatCount="indefinite"/>'


def smil_animate_width(values, dur=TOTAL_DUR):
    """Return an <animate> element for width with discrete calcMode."""
    vals = ";".join(str(v) for v in values)
    return (f'<animate attributeName="width" values="{vals}" '
            f'keyTimes="0;0.333;0.667" calcMode="discrete" '
            f'dur="{dur}s" repeatCount="indefinite"/>')


def smil_animate_fill(values, dur=TOTAL_DUR):
...
def generate_schedule_comment():
    ...
    lines.append(f"  Animation: SMIL <animate> + <set> (no CSS keyframes)")
    lines.append("")
...
def generate_left_panel():
    ...
    lines.append("<!-- Base model box (static rect) -->")
    lines.append('<rect x="22" y="248" width="230" height="106" rx="12" fill="#0F62FE"/>')

    # Base model text — 3 variants, one per slot
    for si, slot in enumerate(SLOTS):
        model = slot["model"]
        minfo = MODELS[model]
        begin = slot_begin(si)
        lines.append(f'<g display="none">')
        lines.append(f'  {smil_set(begin)}')
    ...
    # ── Adapter boxes (4 rows) ──
    # Each row has a single <rect> that animates its fill, plus 3 text variants
    for row_idx in range(4):
        ...
        lines.append(f"<!-- Adapter box row {row_idx} -->")
        # Single rect with animated fill
        lines.append(f'<rect x="22" y="{y}" width="230" height="48" rx="8" fill="{fills[0]}">')
        lines.append(f'  {smil_animate_fill(fills)}')
        lines.append(f'</rect>')

        # Text variants — one per slot, toggled with <set>
...
def generate_right_panel():
    ...
    for si, slot in enumerate(SLOTS):
        ...
        minfo = MODELS[model]
        begin = slot_begin(si)
        lines.append(f'<text display="none" x="530" y="28" text-anchor="middle" font-size="10" fill="#8d8d8d" letter-spacing="1">')
    ...
    for row_idx in range(4):
        ...
        # Single baseline bar rect with animated width
        lines.append(f'<rect x="{BAR_X}" y="{base_y}" width="{base_widths[0]}" height="{BAR_H}" rx="{BAR_RX}" fill="#c6c6c6">')
        lines.append(f'  {smil_animate_width(base_widths)}')
        lines.append(f'</rect>')

        # Single improved bar rect with animated width AND fill
        lines.append(f'<rect x="{BAR_X}" y="{imp_y}" width="{imp_widths[0]}" height="{BAR_H}" rx="{BAR_RX}" fill="{imp_fills[0]}">')
        lines.append(f'  {smil_animate_width(imp_widths)}')
        lines.append(f'  {smil_animate_fill(imp_fills)}')
        lines.append(f'</rect>')

        # Percentage texts — 3 variants (one per slot), toggled with <set>
        ...
        # Row labels + badges — 3 variants (one per slot), toggled with <set>
        for si in range(len(SLOTS)):
            aid = adapter_ids[si]
            info = INTRINSICS[aid]
            lib = LIBS[info["lib"]]
